...
bot.on('text', async (ctx) => {
    ...
    if (state) {
        ...
        if (state.step === 'phone') {
            ...
            // Системный сброс воронки продаж, чтобы при следующем "Привет" ИИ начал с нуля
            const resetMsg = '[СИСТЕМА: Заявка успешно оформлена. Цикл бронирования завершен. Теперь отвечай на новые вопросы или подбирай новые экскурсии с нуля. Не предлагай забронировать ту же самую экскурсию и не проси данные для старой заявки.]';
            await saveMessage(telegramId, 'system', resetMsg);
